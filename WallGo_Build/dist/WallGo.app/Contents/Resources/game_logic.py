...
class GameState:
    def __init__(self):
        ...
        self.stay_in_place_delay = 2000  # 2 seconds in milliseconds
    ...
    def handle_setup(self, row, col):
        ...
        if self.setup_piece_count < 4:
            ...
            self.phase = "select"
            self.current_player = 0  # Red starts
    ...
    def handle_wall_placement(self, mouse_pos, cell_size, margin):
        ...
        # Check if click is within the board boundaries
        if x < 0 or y < 0 or x > cell_size * self.board_size or y > cell_size * self.board_size:
            return  # Click is outside the board
        
        # Get the cell coordinates (floating point for more precise detection)
        col_float = x / cell_size
        row_float = y / cell_size
        ...
        dist_to_horizontal = abs(row_float - round(row_float))
        dist_to_vertical = abs(col_float - round(col_float))
        
        # Threshold for detecting clicks near grid lines (smaller value = more precise)
        threshold = 0.15
        
        piece_row, piece_col = self.pieces[self.current_player][self.selected_piece]
        
        # Determine if click is closer to a horizontal or vertical grid line
    ...
    def has_path(self, start_row, start_col, target_row, target_col):
        ...
        while queue:
            ...
            for dr, dc in directions:
                ...
                if (0 <= new_row < self.board_size and 
                    ...
                    # Skip cells with pieces (except the target)
                    is_occupied = False
                    for player in range(2):
                        for piece in self.pieces[player]:
                            if piece is not None and piece == (new_row, new_col) and piece != (target_row, target_col):
                                is_occupied = True
                                break
        ...
        # If we've exhausted all possibilities without finding the target, there's no path
        return False
    ...
    def has_wall_between(self, row1, col1, row2, col2):
        ...
        if row2 == row1 + 1:  # Moving down
            return self.walls[0][row2][col1]
        elif row2 == row1 - 1:  # Moving up
            return self.walls[0][row1][col1]
        # Check vertical wall
        elif col2 == col1 + 1:  # Moving right
            return self.walls[1][row1][col2]
        elif col2 == col1 - 1:  # Moving left
            return self.walls[1][row1][col1]
            
        return False
    
    def calculate_enclosed_areas(self):
        # Calculate the size of enclosed areas for each piece
        areas = [
            [0, 0],  # Red pieces
            [0, 0]   # Blue pieces
        ]
                ...
                while queue:
                    ...
                    for dr, dc in directions:
                        ...
                        if (0 <= new_r < self.board_size and 
                            ...
                            self.is_valid_move(r, c, new_r, new_c)):
                            
                            # Add to queue and mark as visited
                            queue.append((new_r, new_c))
                            visited.add((new_r, new_c))
                
                # Count the visited cells (including the cell with the piece)
                areas[player][piece_idx] = len(visited)
